...
## Atomovvy fork policy overlay

For Atomovvy-managed repository-changing work in this fork, also read `Documentation/Workflow/REPO_POLICY_MANIFEST.md` and `Documentation/Workflow/AGENT_LAST_RUN.md` together with the current applicable central policy from `Atomovvy/Atomovvy`.

Ordinary hosted GitHub mutations use P2-C Simple Lock v2:
...
This overlay does not weaken or replace any existing UiPath skill, flavor, hook, test, packaging, contribution, or publication rule. The lock is coordination only and does not grant release, package publication, workflow, credential, external-service, or merge authority.
